...
int main(){
    ...
    function<ll(ll)>B = [&](ll i){return i;};
    function<ll(ll)>G = [&](ll i){return i+n;};

    function<ll(ll)>isB = [&](ll i){
        return (1 <= i) && (i<=n);
    };
    function<ll(ll)>isG = [&](ll i){
        return (n<i) && (i<=n+m);
    };
    ...
    rep(i,1,n+1)d.addEdge(s, B(i), 1);
    rep(i,1,m+1)d.addEdge(G(i), t, 1);
    

    cout<<d.maxFlow(s, t)<<'\n';
    vector<vector<ll>>comp = d.minCutComponents(s);
    vector<ll>repr(n+m+3,0);
    ll idx = 0;
    for(vector<ll>&v: comp){
        for(ll node: v)repr[node]=idx;
        idx++;
    }
    // show(repr);
    ...
    rep(i,1,n+1){
        for(auto&e: d.adj[i]){
            if(!isB(i) || !isG(e.to))continue;
            if(e.cap == e.flow)cout<<i<<" "<<e.to-n<<'\n';
        }
    }
    return 0;
}
