...
int main(){
    ...
    vector<ll>dist = shortestPathSingleSource(g, 1);
    for(Edge&e: g.edges){
        if(dist[e.to] == dist[e.from]+e.weight)h.addEdge(e.from, e.to, e.weight);
    }

    // for(Edge&e: h.edges)cout<<e.from<<" "<<e.to<<" "<<e.weight<<'\n';
    vector<ll>topo = toposort(h);
    reverse(all(topo));
    vector<ll>num_ways(n+1,0);
    vector<ll>min_v(n+1,n+1);
    ...
    max_v[n]=0;
    for(ll node: topo){
        for(Edge&e: h.adj[node]){
            if(reached[e.to]){
                num_ways[e.from] += num_ways[e.to];
                num_ways[e.from] %= MOD;
                reached[e.from]=true;
                min_v[e.from] = min(min_v[e.from], 1 + min_v[e.to]);
                max_v[e.from] = max(max_v[e.from], 1 + max_v[e.to]);
            }
        }
    }
    cout<<dist[n]<<" "<<num_ways[1]<<" "<<min_v[1]<<" "<<max_v[1];
