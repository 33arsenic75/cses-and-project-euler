...
int main(){
    ...
    vector<vector<ll>>comp = scc(g);
    vector<ll>repr(n+1,0);
    ll N = comp.size();
    ll idx = 1;
    ...
    for(Edge&e: g.edges){
        // cout<<e.from<<" "<<e.to<<'\n';
        if(repr[e.from]!=repr[e.to])edges[repr[e.from]].insert(repr[e.to]);
    }
    
    Graph h(N,true);
    rep(i,1,N+1){
        for(ll to: edges[i])h.addEdge(i, to, 0);
    }

    vector<ll>dp(N+1,0);
    vector<ll>top = toposort(h);
    rep(i,1,n+1)dp[repr[i]]+=coins[i];
    reverse(all(top));
    // show(dp);
    // for(Edge&e: h.edges){
    //     cout<<e.from<<" "<<e.to<<'\n';
    ...
    for(ll node: top){
        ll best = 0;
        for(Edge&e : h.adj[node])best = max(best, dp[e.to]);
        dp[node] += best;
    }
