...
bool onSegment(geometry<ll> A, geometry<ll> B, geometry<ll> C) {
  return cross(A - C, B - C) == 0 && dot(A - C, B - C) <= 0;
}

geometry<ll> a(1, 0), b(0, 0), c(-1, 0);
double argg = signedAngleABC(a, b, c);
int main() {
  ...
  while (m--) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      sum += signedAngleABC(A, c, B);
    }

    if (boundary)
      cout << "BOUNDARY\n";
    else if (abs(sum) < 1e-6)
      cout << "OUTSIDE\n";
    else
      cout << "INSIDE\n";
  }
