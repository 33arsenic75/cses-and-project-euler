...
vector<ll> modpow(vector<ll>&b, ll exp){
    vector<ll>temp = {1};
    if(exp==0)return temp;
    temp = modpow(b, exp/2);
    temp = multiply(temp, temp);
    if(exp&1)temp = multiply(temp, b);
    return temp;
}
int main(){
    ll n, a, b;
    ...
    // dp[i][j] = probability that sum = j after i dice
    vector<vector<long double>> dp(n + 1, vector<long double>(6 * n + 1, 0.0L));
