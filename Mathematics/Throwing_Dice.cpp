...
struct Matrix {
    ...
    vector<vector<ll>> a;
    ll MOD;
    Matrix(int n_, bool ident = false, ll MOD_ = (1e9)+7) : n(n_) {
...
Matrix operator*(const Matrix &A, const Matrix &B) {
    ...
    for (int i = 0; i < n; i++) {
        for (int k = 0; k < n; k++) {
            ...
            for (int j = 0; j < n; j++) {
                C.a[i][j] = (C.a[i][j] + A.a[i][k] * B.a[k][j]) % C.MOD;
            }
        }
    }
    return C;
...
int main(){
    ll n; 
    cin>>n;
    vector<ll>C = {1, 1, 1, 1, 1, 1};
    vector<ll>A = {1, 1, 2, 4, 8, 16};
    cout<<linear_recurrence(C, A, n);
    return 0;
}
